package carrierOps;

import java.util.HashMap;

public interface Fleet {
	public static HashMap<Integer,String>cvn(){
		HashMap<Integer,String>cvn=new HashMap<>();
		cvn.put(68,"Nimitz");
		cvn.put(69,"Dowight_D_Eisenhower");
		cvn.put(70,"Carl_Vinson");
		cvn.put(71, "Theodoa_Roosevelt");
		cvn.put(72, "Abraham_Loncoln");
		cvn.put(73,"George_Washington");
		cvn.put(74,"John_C_Stennis");
		cvn.put(75, "Harry_S_Truman");
		cvn.put(76, "Ronald_Reagan");
		cvn.put(77, "George_H_W_Bush");
		return cvn;
	}
	public static HashMap<Integer,String>cg(){
		HashMap<Integer,String>cg=new HashMap<>();
		
		return cg;
	}
	public static HashMap<Integer,String>ddg(){
		HashMap<Integer,String>ddg=new HashMap<>();
		
		return ddg;
	}
}
